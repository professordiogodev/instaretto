...
- **`.env` is a file**, sitting in this project's folder, holding
  ...
  `SECRET_KEY`, `DATABASE_URL`, bucket names, and so on. It gets
  passed into the container via `env_file: .env` in
  `docker-compose.yml`, so whatever's in it is available every time
  you run `docker compose up`, no matter which terminal tab you're in.
  You edit it once and it stays edited.
...
1. Copy the env file (this holds configuration and secrets the app
   ...
   **Edit `.env`** (the file you just created — open it in a text
   ...
   - Leave `DATABASE_URL` as the default (it points at the `db`
...
Configure those as a *second*, separate named profile on your host
machine — don't overwrite your main one:
...
(`us-east-1`), and output format (`json` is fine). This profile lives
in `~/.aws`, which `docker-compose.yml` already mounts read-only into
the container — you don't need to do anything container-side for the
app to be able to see it.
...
Then restart the app so it picks up the change:
...
Once you're in, on the instance. This whole block *is* a command (the
`cat > .env <<EOF ... EOF` part writes the file for you), but
**replace `<the-master-password>`, `<rds-endpoint>`, and both
`<yourname>` placeholders with your real values before running it** —
pasting them in literally will write a broken `.env`, the same way
leaving `<yourname>` in a policy file breaks the IAM policy in
Part 2.2:
...
8. You used two different AWS identities in this lab for two different
